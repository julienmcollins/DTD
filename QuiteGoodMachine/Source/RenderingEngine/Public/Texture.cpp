...
#include "QuiteGoodMachine/Source/RenderingEngine/Private/BasicShaderProgram.h"
...
bool Texture::LoadFromFile(const GLchar *file, GLboolean alpha) {
   ...
   image_format = GL_RGBA;
   int width, height;
   image = SOIL_load_image(file, &width, &height, 0, image_format == GL_RGBA ? SOIL_LOAD_RGBA : SOIL_LOAD_RGB);
...
Texture::~Texture() {
   // Free image (this might cause problems)
   // Free();
}